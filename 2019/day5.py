...
def compute(input, noun=None, verb=None):
    ...
    while True:
        ...
        print(f"i:{i} opcodes:{opcodes} opcode:{opcode} outputs:{outputs} p224:{inputs[224:225]} inputs:{inputs}")
        ...
        # where the inputs and outputs are, not their values.
            ...
            p1 = value(inputs, i+1, parameter_mode(opcodes, 1))
            p2 = value(inputs, i+2, parameter_mode(opcodes, 2))
            p3 = value(inputs, i+2, parameter_mode(opcodes, 2))

            print(f"opcode:2 p1{p1} p2{p2}")
            output = p1 * p2
            inputs[inputs[i+3]] = output
            i += 4
